...
import { EvmSchemaUpdatedEvent, EvmSchemaSynchronisedEvent } from '@easylayer/common/domain-cqrs-components/evm';
...
export class Schema extends AggregateRoot {
  ...
  public async sync({
    ...
    if (upQueries.length === 0) {
      ...
      return await this.apply(
        new EvmSchemaSynchronisedEvent({
  ...
  private onEvmSchemaSynchronisedEvent({ payload }: EvmSchemaSynchronisedEvent) {
    const { aggregateId } = payload;
    this.aggregateId = aggregateId;
  }

  private onEvmSchemaUpdatedEvent({ payload }: EvmSchemaUpdatedEvent) {
