...
/**
 * Optimized BlocksQueue for Ethereum/EVM chains with simplified implementation.
 *
 * Key optimizations:
 * 1. O(1) lookups via blockNumber and hash indexes
...
export class BlocksQueue<T extends Block> {
  ...
  /**
   * Creates an instance of BlocksQueue for EVM chains.
   */
  constructor({
    lastHeight,
    maxQueueSize,
  ...
  public async enqueue(block: T): Promise<void> {
    await this.mutex.runExclusive(async () => {
      ...
      // Check height sequence (using blockNumber for EVM)
  ...
  /**
   * Fetches a block by its blockNumber from the queue (async version).
   * @complexity O(1)
   */
  ...
  private cleanupBlockHexData(block: T): void {
    ...
    // Remove transaction hex data to save memory (EVM uses 'transactions' instead of 'tx')
