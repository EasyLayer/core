...
export const createEtherJSProvider = (options: EtherJSProviderOptions): EtherJSProvider => new EtherJSProvider(options);
...
export class EtherJSProvider extends BaseNodeProvider<EtherJSProviderOptions> {
  ...
  get connectionOptions(): EtherJSProviderOptions {
    return {
      type: this.type,
  ...
  private initializeWebSocket(): void {
    ...
    try {
      ...
      this._wsClient.websocket.on('error', () => {
        this._isWebSocketConnected = false;
      });
    } catch {
      this._isWebSocketConnected = false;
  ...
  healthcheckWebSocket(): boolean {
    if (!this._hasWebSocketUrl || !this._wsClient) return false;
    const ws = this._wsClient.websocket;
    return ws && ws.readyState === ws.READY_STATE_OPEN && this._isWebSocketConnected;
  }

  async handleConnectionError(error: any, _methodName: string): Promise<void> {
    throw error;
  }

  public async healthcheck(): Promise<boolean> {
    try {
      const results = await this.rateLimiter.execute([{ method: 'eth_blockNumber', params: [] }], (calls) =>
  ...
  public async connect(): Promise<void> {
    ...
    try {
      const raw = await this._httpClient.send('eth_chainId', []);
      const parsed = parseInt(raw, 16);
      if (parsed !== this.network.chainId) {
        throw new Error(`Chain ID mismatch: expected ${this.network.chainId}, got ${parsed}`);
      }
    } catch (e) {
  ...
  public async reconnectWebSocket(): Promise<void> {
    if (!this._hasWebSocketUrl || !this.wsUrl) throw new Error('No WebSocket URL');
    if (this._wsClient) {
      this._wsClient.destroy();
      this._wsClient = undefined;
      this._isWebSocketConnected = false;
  ...
  private async _batchRpcCall(calls: Array<{ method: string; params: any[] }>): Promise<any[]> {
    ...
    return (Array.isArray(results) ? results : [results]).map((r: any) => {
      ...
      if (r.error) throw new Error(`JSON-RPC Error ${r.error.code}: ${r.error.message}`);
      return r.result;
    });
  }
  ...
  private async _batchWsCall(calls: Array<{ method: string; params: any[] }>): Promise<any[]> {
    ...
    const results = await Promise.all(calls.map((c) => this._wsClient!.send(c.method, c.params)));
  ...
  private get batchCall() {
    return this._wsClient && this.healthcheckWebSocket()
      ? (calls: any) => this._batchWsCall(calls)
      : (calls: any) => this._batchRpcCall(calls);
  }
  ...
  public subscribeToNewBlocks(callback: (blockNumber: number) => void): { unsubscribe: () => void } {
    ...
    return {
      unsubscribe: () => {
        if (this._wsClient) this._wsClient.off('block', callback);
      },
    };
  }
  ...
  public async getManyBlocksByHeights(
    ...
    return this.executeWithErrorHandling(async () => {
      ...
      return Promise.all(
        rawBlocks.map(async (block, i) => {
          if (!block || block.error) return null;
          if (verifyTrie && fullTransactions && block.transactions?.length && block.transactionsRoot) {
            const valid = await EvmTrieVerifier.verifyTransactionsRoot(block.transactions, block.transactionsRoot);
            if (!valid) throw new Error(`Transactions root mismatch for block ${heights[i]}`);
          }
          const nb = this.normalizeRawBlock(block);
  ...
  public async getManyBlocksByHashes(hashes: Hash[], fullTransactions = false): Promise<(UniversalBlock | null)[]> {
    ...
    return this.executeWithErrorHandling(async () => {
      ...
      const rawBlocks = await this.rateLimiter.execute(requests, this.batchCall);
      return rawBlocks.map((b) => (!b || b.error ? null : this.normalizeRawBlock(b)));
  ...
  /**
   * Tries debug_traceBlockByNumber (Geth) → trace_block (Erigon/OE).
   * Returns [] if neither is supported.
   */
  ...
  public subscribeToPendingTransactions(callback: (txHash: string) => void): { unsubscribe(): void } {
    if (!this._wsClient || !this.healthcheckWebSocket()) {
      throw new Error('WebSocket not available for pending transactions subscription');
    }
    const handler = (txHash: string) => callback(txHash);
    this._wsClient.on('pending', handler);
    return {
      unsubscribe: () => {
        if (this._wsClient) this._wsClient.off('pending', handler);
      },
    };
  }
  ...
  public async getRawMempool(): Promise<Record<string, any>> {
    try {
      const results = await this._batchRpcCall([{ method: 'txpool_content', params: [] }]);
  ...
  public async getTransactionByHash(hash: string): Promise<UniversalTransaction | null> {
    try {
      ...
      return this.normalizeRawTransaction(results[0]);
    } catch {
      return null;
    }
  }
  ...
  private normalizeGethTrace(raw: any, blockNumber?: number, txHash?: string): UniversalTrace {
    return {
      transactionHash: txHash || raw.txHash || raw.transactionHash || '',
      transactionPosition: raw.index ?? 0,
      type: (raw.type || 'call').toLowerCase(),
      action:
        raw.from !== undefined ? { from: raw.from, to: raw.to, value: raw.value, gas: raw.gas, input: raw.input } : raw,
