...
export class ConnectionManager implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    ...
    providers.forEach((provider: BaseNodeProvider) => {
      ...
      this.log.debug('Blockchain provider registered', {
        args: {
          providerName: name,
        ...
        if (await this.tryConnectProvider(provider)) {
          ...
          this.log.info(`Connected to provider: ${provider.constructor.name}`, {
            args: {
              ...
              hasWebSocket: provider.hasWebSocketSupport,
              wsConnected: provider.isWebSocketConnected,
            },
          });
          return;
        }
      } catch (error: any) {
        this.log.warn('Blockchain provider connection failed, trying next', {
          args: {
  ...
  public async handleProviderFailure(providerName: string, error: any, methodName: string): Promise<BaseNodeProvider> {
    ...
    if (attempts < this.maxReconnectionAttempts) {
      ...
      try {
        // Handle WebSocket-specific reconnection
  ...
  private async switchToNextAvailableProvider(): Promise<BaseNodeProvider> {
    ...
    for (let i = 1; i <= allProviders.length; i++) {
      ...
      this.log.debug('Attempting to switch to provider', {
        args: {
        ...
        if (await this.tryConnectProvider(nextProvider)) {
          ...
          this.log.info('Successfully switched to backup provider', {
            args: {
              ...
              hasWebSocket: nextProvider.hasWebSocketSupport,
              wsConnected: nextProvider.isWebSocketConnected,
            },
          });
