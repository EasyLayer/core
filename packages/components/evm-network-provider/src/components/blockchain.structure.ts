...
export class Blockchain {
  ...
  get lastBlockHeight(): number | undefined {
    if (this._tail) {
      return this._tail.block.number;
    }
  }
   ...
   * Complexity: O(1)
   */
  public addBlock(number: number, hash: string, parentHash: string, transactions: string[]): boolean {
    // Before adding a block, we validate it
    if (!this.validateNextBlock(number, parentHash)) {
      return false;
    }

    const newBlock: LightBlock = { number, hash, parentHash, transactions };
   ...
   * Complexity: O(1)
   */
  public validateNextBlock(number: number, parentHash: string): boolean {
    if (!this._tail) {
    ...
    if (this._tail.block.number + 1 !== number) {
      return false;
    }
  ...
  public validateNextBlocks(blocks: LightBlock[]): boolean {
    ...
    if (!this.validateNextBlock(firstBlock.number, firstBlock.parentHash)) {
    ...
    for (let i = 1; i < blocks.length; i++) {
      ...
      if (currentBlock.number !== prevBlock.number + 1 || currentBlock.parentHash !== prevBlock.hash) {
   ...
   *
   * @param {number} number - The height up to which the chain should be truncated.
   ...
   */
  public truncateToBlock(number: number): boolean {
    if (number < -1) {
    ...
    if (this._tail.block.number === number) {
      return true;
    }
    ...
    while (currentNode) {
      if (currentNode.block.number === number) {
    ...
    if (!found) {
      ...
      const currentHeadHeight = this._head?.block.number ?? Infinity;
      if (number < currentHeadHeight) {
        if (this._head) {
  ...
  public validateChain(): boolean {
    ...
    while (current && current.next) {
      ...
      if (nextBlock.number !== current.block.number + 1) {
   ...
   * @param {number} number - The expected height of the last block.
   * @param {string} hash - The expected hash of the last block.
   * @param {string} parentHash - The expected previous hash of the last block.
   ...
   * Complexity: O(1)
   */
  public validateLastBlock(number: number, hash: string, parentHash: string): boolean {
    if (!this._tail) {
   ...
   * @param {number} number - The height of the block to find.
   * @returns {LightBlock | null} The block, or null if not found.
   * Complexity: O(n), where n - is the number of blocks in the chain
   */
  public findBlockByHeight(number: number): LightBlock | null {
