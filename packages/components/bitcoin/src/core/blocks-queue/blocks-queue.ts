...
export class BlocksQueue {
  ...
  public get lastHash(): string | undefined {
    return this._lastHash;
  }

  public setLastHashForContinuity(hash: string | undefined): void {
    this._lastHash = hash;
  }

  public get queueMode(): 'js-buffer' {
    return 'js-buffer';
  }
  ...
  public async enqueue(item: RawBlock): Promise<void> {
    await this.mutex.runExclusive(async () => {
      ...
      if (item.height !== this._lastHeight + 1) {
        throw new Error(`Can't enqueue block. Block height: ${item.height}, Queue last height: ${this._lastHeight}`);
      }
  ...
  /**
   * Retrieves a batch of RawBlocks in FIFO order up to specified size.
