...
export class SubscribeBlocksProviderStrategy implements BlocksLoadingStrategy {
  ...
  public async load(currentNetworkHeight: number): Promise<void> {
    ...
    if (this.queue.lastHeight < currentNetworkHeight && this.queue.maxBlockHeight < currentNetworkHeight) {
      ...
      this._subscription = await this.networkProvider.subscribeToNewBlocks((block: any) => this.enqueueBlocks([block]));
    }
  }
